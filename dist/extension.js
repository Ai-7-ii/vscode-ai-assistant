...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('intelliflow.chatView', provider, { webviewOptions: { retainContextWhenHidden: true } }));
    // Команды
    const commands = [
        vscode.commands.registerCommand('intelliflow.setApiKey', async () => {
            ...
            if (apiKey) {
                await vscode.workspace.getConfiguration().update('intelliflow.apiKey', apiKey, vscode.ConfigurationTarget.Global);
...
class ChatViewProvider {
    ...
    resolveWebviewView(webviewView, context, _token) {
        ...
        // ОБРАБОТКА СООБЩЕНИЙ ОТ WEBVIEW - ВАЖНОЕ ИСПРАВЛЕНИЕ!
        webviewView.webview.onDidReceiveMessage(async (data) => {
            console.log('📩 Получено сообщение от webview:', data.type);
            switch (data.type) {
                case 'sendMessage':
                ...
                case 'getApiKey':
                    ...
                    break;
                case 'quickAction':
                    await this._handleQuickAction(data.action);
                    break;
                // ВАЖНОЕ ДОПОЛНЕНИЕ: Обработка кнопки шестерёнки!
                case 'openSettings':
                    console.log('🛠️ Получен запрос на открытие настроек от кнопки ⚙️');
                    // Отправляем команду обратно в webview чтобы показать панель настроек
                    if (this._view) {
    ...
    async _handleUserMessage(text) {
        ...
        try {
            const apiKey = vscode.workspace.getConfiguration().get('intelliflow.apiKey');
            if (!apiKey) {
                this._view.webview.postMessage({
                    type: 'error',
                    error: 'API key not set. Click the gear icon ⚙️ to set your DeepSeek API key.'
                });
                return;
            }
    ...
    async _handleQuickAction(action) {
        ...
        if (prompt) {
            this.sendMessageToWebview(prompt);
        }
    }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #sendButton:hover { background: #1177bb; }
        #sendButton:disabled { 
            background: #2d2d30; 
            cursor: not-allowed; 
        }
...
<body>
    ...
    <div class="settings-panel" id="settingsPanel">
        <h3>API Settings</h3>
        <p>Get your API key from <a href="https://platform.deepseek.com" target="_blank">DeepSeek Platform</a></p>
        <input type="password" class="api-key-input" id="apiKeyInput" placeholder="Enter your DeepSeek API key">
    ...
    <div class="chat-container" id="chatContainer">
        <div class="message ai-message">
            ...
            <div class="message-content">
                Hello! I'm your AI coding assistant powered by DeepSeek.
                <br><br>
                <strong>How to use:</strong><br>
                1. Click ⚙️ to set your API key<br>
                2. Type messages below<br>
                3. Use commands from right-click menu on code<br>
                <br>
                <strong>Commands:</strong><br>
                • Right-click code → Explain Code<br>
                • Right-click code → Refactor Code<br>
                • Right-click code → Fix Code<br>
                • Right-click code → Generate Code
            </div>
        </div>
    </div>
    
    <div class="quick-actions">
        <button class="action-btn" onclick="handleQuickAction('explain')">Explain Code</button>
        <button class="action-btn" onclick="handleQuickAction('refactor')">Refactor Code</button>
        <button class="action-btn" onclick="handleQuickAction('fix')">Fix Errors</button>
        <button class="action-btn" onclick="handleQuickAction('generate')">Generate Code</button>
    </div>
    ...
    <div class="input-container">
        <input type="text" id="messageInput" placeholder="Type your message or command...">
    ...
    <script>
        ...
        // КНОПКА НАСТРОЕК - ОСНОВНОЕ ИСПРАВЛЕНИЕ!
        if (settingsBtn) {
            settingsBtn.addEventListener('click', () => {
                console.log('⚙️ Settings button clicked - sending message to extension');
                ...
                // Локальное переключение (на всякий случай)
                if (settingsPanel) {
        ...
        window.addEventListener('message', (event) => {
            ...
            switch (message.type) {
                case 'addMessage':
                    addMessage(message.message.content, message.message.role === 'user');
                    break;
                ...
                case 'apiKeyInfo':
                    if (apiKeyStatus) {
                        if (message.hasKey) {
                            apiKeyStatus.innerHTML = \`✅ API key saved: \${message.maskedKey}\`;
                            if (apiKeyInput) apiKeyInput.value = '';
                        } else {
                            apiKeyStatus.innerHTML = '❌ No API key set';
                        }
                    }
                    break;
                ...
                // ВАЖНОЕ ДОПОЛНЕНИЕ: Обработка команды от расширения
                case 'showSettingsPanel':
                    console.log('🛠️ Received command to show settings panel');
                    if (settingsPanel) {
                        settingsPanel.style.display = message.show ? 'block' : 'none';
                        if (message.show) {
                            vscode.postMessage({ type: 'getApiKey' });
                            setTimeout(() => {
                                if (apiKeyInput) {
